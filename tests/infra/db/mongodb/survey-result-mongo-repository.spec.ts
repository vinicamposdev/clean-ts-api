...
describe('SurveyMongoRepository', () => {
  ...
  describe('loadBySurveyId()', () => {
    test('Should load survey result', async () => {
      ...
      const sut = makeSut()
      const surveyResult = await sut.loadBySurveyId({ accountId, surveyId: survey.id })
      expect(surveyResult).toBeTruthy()
    ...
    test('Should load survey result 2', async () => {
      ...
      await surveyResultCollection.insertMany([{
        ...
        surveyId: new ObjectId(survey.id),
        accountId: new ObjectId(account3),
        answer: survey.answers[1].answer,
        date: new Date()
      }])
      const sut = makeSut()
      const surveyResult = await sut.loadBySurveyId({ accountId: account2, surveyId: survey.id })
      expect(surveyResult).toBeTruthy()
      expect(surveyResult.surveyId).toEqual(survey.id)
      expect(surveyResult.answers[0].count).toBe(2)
    ...
    test('Should load survey result 3', async () => {
      ...
      const sut = makeSut()
      const surveyResult = await sut.loadBySurveyId({ accountId: account3, surveyId: survey.id })
      expect(surveyResult).toBeTruthy()
      expect(surveyResult.surveyId).toEqual(survey.id)
